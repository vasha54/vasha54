...
<img src="https://github.com/vasha54/vasha54/blob/main/img/git.svg" title="Git"  width="50" height="50">*Proficient in the Git version control system for efficient source code management. Experienced in creating and managing local and remote repositories, branch handling (`branch`, `checkout`, `merge`), conflict resolution, version tagging (`tag`), and configuration of collaborative workflows. Skilled in using inspection commands (`log`, `status`, `diff`) for precise tracking of change history. Capable of structuring atomic, meaningful commits, enhancing maintainability and team collaboration.*

<img src="https://github.com/vasha54/vasha54/blob/main/img/github-actions.svg" title="GitHub Actions" width="50" height="50"> *Experienced in using GitHub as a hosting and collaboration platform, including issue management, pull requests, and code reviews. Strong knowledge of **GitHub Actions** for implementing Continuous Integration and Continuous Deployment (CI/CD) pipelines. Capable of designing custom workflows triggered by events (`on: push`, `workflow_dispatch`), automating tasks such as documentation generation, testing, artifact building, and deployment. Integration of marketplace actions and execution of complex scripts directly on GitHub runners, optimizing development and software delivery processes.*

<img src="https://github.com/vasha54/vasha54/blob/main/img/sed.svg" title="sed" width="50" height="50"> *Proficiency in regular expressions for dynamic transformation of configuration files and Markdown documents. Use of `sed -E` to replace complex patterns (GitHub URLs) with local paths, enabling document portability between development and production environments.*

<img src="https://github.com/vasha54/vasha54/blob/main/img/md-to-pdf.png" title="md-to-pdf" width="50" height="50"> *Experience using `md-to-pdf` (Puppeteer‑based) to convert Markdown files into professional PDF documents. Advanced configuration of launch options (`--launch-options`), handling of sandbox‑restricted environments (`--no-sandbox`), relative path resolution (`--basedir`), and output file customization (`--output`). Troubleshooting local image rendering and CSS style compatibility.*
