...
<img src="https://github.com/vasha54/vasha54/blob/main/img/bash.png" title="Bash"  width="50" height="50"> *Extensive experience in developing automation scripts for Linux environments, ranging from Odoo module structure generation and implementation of robust backup systems for platforms such as Moodle and MySQL/PostgreSQL databases, to automation of document processing tasks. Proficient in file and directory manipulation (mkdir, touch, cp, tar), with conditional logic (if) and loops (for, while) to process multiple elements dynamically. Skilled in creating reusable functions with parameter passing, line‑by‑line file processing, and programmatic generation of content in Python (__manifest__.py) and XML formats. Expertise in designing detailed logging systems that record every execution step, facilitating auditing and troubleshooting. Experienced in managing incremental backups using tar and database dumps with mysqldump (MySQL) and pg_dump (PostgreSQL), including integrity verification and error redirection. Proficient in enforcing automatic retention policies with find. Additionally, skilled in image and document processing tools such as sam2p for JPG to PDF conversion, pdftk for PDF concatenation, and pdfjam for merging documents with custom cover pages. Competent in setting recursive permissions (chmod / umask) and handling command‑line arguments with strict validation, ensuring script robustness and reliability in production environments.*
